...
def fixture(path, content, crop=False, unicode_map=None):
    objects = [b"<< /Type /Catalog /Pages 2 0 R >>", b"<< /Type /Pages /Kids [3 0 R 4 0 R 5 0 R 6 0 R] /Count 4 >>"]
    for rotation in (0, 90, 180, 270):
        box = "/CropBox [40 60 360 540]" if crop else ""
        objects.append(
            (
                f"<< /Type /Page /Parent 2 0 R /MediaBox [0 0 400 600] {box} "
                f"/Rotate {rotation} /Resources << /Font << /F1 8 0 R >> >> /Contents 7 0 R >>"
            ).encode()
        )
...
class Worker:
    def __init__(self, path):
        self.process = subprocess.Popen(
            [str(ROOT / ".build/pdfpreview-native"), str(path)],
...
def check_text(directory):
    ...
    with Worker(pdf) as worker:
        ...
        for n, geometry in enumerate(pages, 1):
            ...
            worker.checked(
                page=n, px=width, py=height, x=0, y=0, width=width, height=height, format=32, file=str(output)
            )
    ...
    with Worker(pdf) as worker:
        assert worker.checked(action="text", page=1)["text_page"]["characters"] == [], (
            "Image/vector-only pages are not OCRed"
        )
    print(
        "PASS: PDFKit characters, multiline indices, rotated CropBox alignment, glyph coverage, Unicode clusters and empty pages"
    )
...
def check_rasters(directory, pdf):
    with Worker(pdf) as worker:
        ...
        for page in range(1, 5):
            ...
            region = dict(
                page=page, px=2100, py=2800, x=987, y=1001, width=1101, height=1081, format=32, file=str(crop)
            )
...
def check_selections(directory, pdf):
    with Worker(pdf) as worker:
        ...
        rectangles = [dict(x1=-5.5, y1=20.3, x2=143.2, y2=40.1), dict(x1=180.6, y1=230.8, x2=600, y2=500)]
        for action in actions:
            request = dict(
                action=action, height=height, pages=[page], parts=[dict(width=width, offset=0, file=str(output))]
            )
            ...
            worker.checked(
                **dict(
                    request,
                    parts=[
                        dict(width=117, offset=0, file=str(stripes[0])),
...
def check_metal(directory, pdf):
    with Worker(pdf) as worker, Worker(pdf) as reference:
        ...
        reference.checked(
            page=1, px=384, py=512, x=0, y=0, width=384, height=512, format=32, file=str(source), cache=False
        )
        ...
        worker.checked(
            ...
            height=height,
            pages=[page],
            parts=[dict(width=88, offset=0, file=str(parts[0])), dict(width=159, offset=88, file=str(parts[1]))],
        )
...
def check_refined_selection(directory, pdf):
    ...
    with Worker(edge_pdf) as worker:
        ...
        parts = [
            dict(width=w, offset=sum(widths[:i]), file=str(directory / f"highlight-{i}.rgba"))
        ...
        for left, right in [(15, 70), (100, 420), (252, 278), (0, 660), (660, 660), (15, 70)]:
            ...
            if masks:
                ...
                expected[40:90, left:right, :3] = (
                    region * 165 + np.array([64, 140, 255], dtype=np.uint16) * 90 + 127
                ) // 255
            assert np.array_equal(read(), expected), (
                "Highlight-only redraw keeps every high-resolution pixel; tint never accumulates"
            )
        ...
        for invalid in [
            dict(selection_cache="missing"),
            dict(height=height + 1),
        ...
        assert worker.checked(**uncached)["selection_cache_bytes"] == 0, (
            "Uncached refinement releases its retained base"
        )
    print(
        "PASS: full-resolution drag pixels, striped masks, exact clearing, bounded clean cache, stale keys and geometry validation"
    )
...
with tempfile.TemporaryDirectory(prefix="pdfpreview-refine-pixels-") as directory:
    ...
    try:
        ...
        for page in range(1, 5):
            ...
            assert "error" not in worker.request(
                page=page,
                px=1200,
                py=1800,
                x=119,
            ...
            assert diff.max() <= 1 and diff.mean() < 0.001, (page, diff.max(), diff.mean())
        ...
        pages = [
            dict(page=1, left=17.25, top=-400.75, width=411.5, height=617.25),
        ...
        assert "error" not in worker.request(
            action="refine", height=height, pages=pages, parts=[dict(width=width, offset=0, file=str(paths[0]))]
        )
        assert "error" not in worker.request(
            action="refine",
            height=height,
            pages=pages,
            parts=[dict(width=239, offset=0, file=str(paths[1])), dict(width=401, offset=239, file=str(paths[2]))],
        )
        whole = np.frombuffer(paths[0].read_bytes(), dtype=np.uint8).reshape(height, width, 4)
        joined = np.concatenate(
            [
                np.frombuffer(p.read_bytes(), dtype=np.uint8).reshape(height, w, 4)
                for p, w in zip(paths[1:], [239, 401])
            ],
            axis=1,
        )
        delta = np.abs(whole.astype(np.int16) - joined.astype(np.int16))
        assert delta.max() <= 1 and delta.mean() < 0.001, (delta.max(), delta.mean())
        ...
        worker.checked(
            action="refine", height=2049, pages=[valid_page], parts=[dict(width=4096, offset=0, file=str(large))]
        )
        assert large.stat().st_size == 4096 * 2049 * 4
        rejected = worker.request(
            action="refine", height=4097, pages=[valid_page], parts=[dict(width=4096, offset=0, file=str(unused))]
        )
    ...
    try:
        page = dict(page=1, width=24000, height=36000, left=-12000 + 50.25, top=-17000, px=2731, py=4096)
        width, height = 101, 10
        fine, coarse = directory / "edge-fine.rgba", directory / "edge-coarse.rgba"
        reply = worker.request(
            action="refine", height=height, pages=[page], parts=[dict(width=width, offset=0, file=str(fine))]
        )
        assert "error" not in reply, reply
        ...
        if info.get("surface"):
            reply = worker.request(
                action="compose", height=height, pages=[page], parts=[dict(width=width, offset=0, file=str(coarse))]
            )
            assert "error" not in reply, reply
            ...
            print(
                f"Large vector edge: {coarse_gray} intermediate columns from cached scaling, {fine_gray} from refinement"
            )
        assert fine.stat().st_size == width * height * 4, "A large page allocates only visible output pixels"
    finally:
        worker.close()
print(
    "PASS: viewport refinement, rotated CropBox, direct crop agreement, fractional stripes, gaps, opaque pixels, bounded outputs, invalid requests, and large-scale vector detail"
)
